...
def split_text(text, max_tokens, queue=None):
    """Split text into smaller chunks ensuring that each chunk ends at a sentence boundary."""
    log = lambda msg: queue.put(msg) if queue else print(msg)
    ...
    log(f"Calculating splits: {total_lines} lines")
    ...
    for i, line in enumerate(lines):
        ...
        display_progress_bar(progress, prefix='Splitting', queue=queue)
...
def display_progress_bar(progress, prefix='', length=40, suffix='', queue=None):
    """Display a simple progress bar."""
    log = lambda msg, end=None, flush=None: queue.put(msg) if queue else print(msg, end=end, flush=flush)

    # Simplified output for queue-based logging
    if queue:
        log(f"{prefix}: {int(progress * 100)}% {suffix}")
        return

    block = int(round(length * progress))
    bar = '||' + '+' * block + '=' * (length - block) + '||'
    log(f'\r{prefix}: {bar} {int(progress * 100)}% {suffix}', end='', flush=True)
...
def translate_full(full_text, input_lang, target_lang, client):
    ...
    system_prompt = (
        ...
        f"Your task is to **accurately translate** the text enclosed between the 🔤 symbols from {input_lang_full} to {target_lang_full}. "
        ...
        f"Deliver the translation exactly as required, without any additional commentary or explanation, and ensure the 🔤 symbols are removed in the final output."
    )
...
def translate_file(input_path, output_path, base_lang, target_lang, client, queue=None):
    log = lambda msg: queue.put(msg) if queue else print(msg)
    ...
    try:
        ...
        with open(input_path, "r", encoding="utf-8") as f:
            file_content = f.read()
        elapsed_time = time.time() - start_time
        log(f"File read step: {elapsed_time:.2f} seconds")
    except FileNotFoundError:
        log(f"Input file not found: {input_path}")
        return
    except UnicodeDecodeError:
        log(f"Could not decode file {input_path} using utf-8 encoding.")
        return

    # Progress bar for simulation slice calculation
    simulate_progress_bar("Preparing to split text", queue=queue)

    start_time = time.time()
    chunks = split_text(file_content, API_MAX_TOKENS, queue=queue)
    elapsed_time = time.time() - start_time
    ...
    for i, chunk in enumerate(chunks):
        ...
        display_progress_bar(progress, prefix='Translating chunks', suffix=suffix, queue=queue)
    ...
    try:
        ...
        elapsed_time = time.time() - start_time
        log(f"File write step: {elapsed_time:.2f} seconds")
        log(f"Translation saved to {output_path}")
    except IOError:
        log(f"Cannot write to output file: {output_path}")

def simulate_progress_bar(description, duration=0.5, queue=None):
    """Simulates a progress bar for a given duration."""
    log = lambda msg: queue.put(msg) if queue else print(msg)
    log(description)
    for i in range(101):
        display_progress_bar(i / 100.0, prefix=description, queue=queue)
...
def process_directory(input_dir, output_dir, base_lang, target_lang, recursive, client, queue=None):
    log = lambda msg: queue.put(msg) if queue else print(msg)
    # Scan folders and show the number of files
    
    log("Scanning directory for markdown files...")
    all_files = scan_directory(input_dir)
    
    total_files = len(all_files)
    log(f"Total markdown files found: {total_files}")

    for i, file_path in enumerate(all_files):
        progress = (i + 1) / total_files
        display_progress_bar(progress, prefix='Processing files', queue=queue)
        ...
        translate_file(file_path, output_path, base_lang, target_lang, client, queue=queue)
